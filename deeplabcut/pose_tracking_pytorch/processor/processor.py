    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for n_iter, (anchor, pos, neg) in enumerate(train_loader):
            ...
            if (n_iter + 1) % log_period == 0:
                logger.info(
                    f"Epoch[{epoch}] Iteration[{n_iter + 1}/{len(train_loader)}] Loss: {loss_meter.avg:.3f}, , Base Lr: {scheduler._get_lr(epoch)[0]:.2e}"
                )
        ...
        if cfg["dist_train"]:
            ...
            logger.info(
                f"Epoch {epoch} done. Time per batch: {time_per_batch:.3f}[s] Speed: {train_loader.batch_size / time_per_batch:.1f}[samples/s]"
            )

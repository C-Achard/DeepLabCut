...
class SkeletonBuilder(QtWidgets.QDialog):
    def __init__(self, config_path, parent=None):
        ...
        if not found:
            warnings.warn(
                ...
                f"{', '.join(self.bpts[missing])} will need to be manually connected in the config.yaml.", stacklevel=2
            )
    ...
    def export(self, *args):
        ...
        if len(unconnected):
            warnings.warn(
                "You didn't connect all the bodyparts (which is fine!). This is just a note to let you know.", stacklevel=2
            )
